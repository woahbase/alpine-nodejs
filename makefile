...
RUNFLAGS   := -e PGID=$(PGID) -e PUID=$(PUID) # -c 64 -m 32m
...
test :
	docker run --rm -it $(NAMEFLAGS) $(RUNFLAGS) $(PORTFLAGS) $(MOUNTFLAGS) $(OTHERFLAGS) $(IMAGETAG) sh -ec 'node --version; npm --version; npx --version; yarn --version; git --version'
